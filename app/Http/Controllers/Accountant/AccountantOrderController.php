...
class AccountantOrderController extends Controller
{
    ...
    //==================Show Detail Order Form=======================//
      public function detail_order($id)
      {
        //   $store_quantity = Item::where('store_quantity',$store_quantity)-get();
          ...
          return view('accountant.home.order.detail_order',compact('items','order'));
      }
    //==================End Method=======================//
    ...
    public function delete_order($id_order) {
        ...
        if ($order) {
            ...
            if ($order->delivery_status === 'Completed') {
                return redirect('/accountant/order')->with("error", "Order has already been Completed and cannot be deleted.");
            }
